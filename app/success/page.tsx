...
import { CheckCircle, Package, Truck, ArrowRight, Copy, Shield, User, Clock, Calendar, Hash } from "lucide-react";
...
function Confetti() {
  ...
  useEffect(() => {
    const colors = ["#d4af37", "#dc2626", "#2d8b4e", "#b8860b", "#e8e0d0"];
...
function SuccessContent() {
  ...
  const trackingSteps = [
    ...
    { icon: <CheckCircle className="w-4 h-4" />, text: "Dukun mulai ritual", time: "Proses...", active: true },
    { icon: <Truck className="w-4 h-4" />, text: "Santet dalam perjalanan", time: "Menunggu", active: false },
    { icon: <Package className="w-4 h-4" />, text: "Santet sampai di target", time: "Estimasi 3-7 hari", active: false },
  ];
      ...
      <div className="min-h-screen pt-24 pb-16 flex items-center justify-center">
        <div className="max-w-md mx-auto px-4 w-full">
          ...
          >
            ...
            <h1 className="font-['Cinzel'] text-2xl font-bold text-accent mb-1.5">
              Pembayaran Berhasil!
            </h1>
            ...
            <div className="bg-background/50 dark:bg-[#0a0a0f]/60 rounded-xl p-4 mb-5 text-left">
              ...
              <div className="text-center pb-3 mb-3 border-b border-border border-dashed">
                <p className="text-[11px] text-muted-light/50 uppercase tracking-wider mb-1">Resi Pengiriman</p>
                <code className="text-accent font-mono text-base tracking-wider font-bold">{ref}</code>
                <button
                  onClick={handleCopy}
                  className="ml-2 p-1 hover:bg-accent/10 rounded transition-colors inline-flex align-middle"
                >
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-base font-bold pt-2.5 border-t border-border">
                  ...
                  <span className="text-accent">{amount > 0 ? formatCurrency(amount) : "Rp 0"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-secondary/5 dark:bg-secondary/[0.06] border border-secondary/10 dark:border-secondary/15 rounded-xl p-3 mb-5">
              <p className="text-[12px] text-muted italic">{randomMessage}</p>
            </div>
                  ...
                  <div key={i} className="flex items-start gap-3">
                    <div className="flex flex-col items-center">
                      ...
                      {i < trackingSteps.length - 1 && (
                        <div className={`w-px h-5 ${step.active ? "bg-accent/20" : "bg-muted-light/10"}`} />
                      )}
                    </div>
